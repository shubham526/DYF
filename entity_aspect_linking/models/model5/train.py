...
def main():
    ...
    parser.add_argument('--train-entity-emb', help='Train entity embeddings.', type=str)
    parser.add_argument('--dev-entity-emb', help='Dev entity embeddings.', type=str)
    parser.add_argument('--entity-emb', help='Query-independent entity embeddings.', type=str)
    ...
    if args.entity_emb_type == 'specific':
        if args.train_entity_emb is None:
            raise RuntimeError('Train entity embedding file missing.')
        elif args.dev_entity_emb is None:
            raise RuntimeError('Dev entity embedding file missing.')
        else:
            print('Loading train entity embeddings...')
            train_entity_embeddings = torch.load(args.train_entity_emb, map_location=device)
            print('[Done].')

            print('Loading dev entity embeddings...')
            dev_entity_embeddings = torch.load(args.dev_entity_emb, map_location=device)
            print('[Done].')
    ...
    if args.entity_emb_type == 'specific':
        ...
        train_set = AspectLinkDataset(
            ...
            tokenizer=tokenizer,
            train=True,
            entity_embeddings=train_entity_embeddings,
        ...
        dev_set = AspectLinkDataset(
            ...
            tokenizer=tokenizer,
            train=False,
            entity_embeddings=dev_entity_embeddings,
            max_len=args.max_len,
            data_type=args.model_type
        )
        print('[Done].')
    ...
    #     print('Number of CUDA devices available = ' + str(torch.cuda.device_count()))
