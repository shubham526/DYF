        ...
        if not is_empty(batch):
            if self._train:
                if self._data_type == 'pairwise':
                    ...
                    aspect_features_pos = torch.tensor([item['aspect_features_pos'] for item in batch if item])
                    aspect_features_neg = torch.tensor([item['aspect_features_neg'] for item in batch if item])
                    ...
                    aspect_features = torch.tensor([item['aspect_features'] for item in batch if item])
