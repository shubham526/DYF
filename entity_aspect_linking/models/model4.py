...
class AspectLinkModel(nn.Module):
    ...
    def forward(
        ...
        #     self.context_word_encoder(input_ids.to(self.device), attention_mask.to(self.device))
        #     for input_ids, attention_mask in
        #     zip(context_text_input_ids, context_text_attention_mask)
        # ]
        #
        # aspect_word_embeddings = [
        #     self.aspect_word_encoder(input_ids.to(self.device), attention_mask.to(self.device))
        #     for input_ids, attention_mask in
        #     zip(aspect_text_input_ids, aspect_text_attention_mask)
        # ]
        ...
        # aspect_word_scores = [
        #     self.score(emb1, emb2) for emb1 in aspect_word_embeddings for emb2 in context_word_embeddings
        # ]


        # Score of aspect entity = sum of sim between aspect_ent_emb and each context_ent_emb
        # Score of aspect = sum of scores of each entity in aspect
        # Alternative: Find context specific aspect entity embedding
            # context_emb = mean of emb of all context_ent_emb
            # Score of aspect entity = sim between aspect_ent_emb and each context_emb
            # Score of aspect = sum of scores of each entity in aspect
