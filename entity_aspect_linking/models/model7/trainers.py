        ...
        def train_step(train_batch):
            ...
            if self._model_type == 'pairwise':
                batch_score_pos = self._model(
                    ...
                    attention_mask=train_batch['attention_mask_pos'].to(self._device),
                    token_type_ids=train_batch['token_type_ids_pos'].to(self._device),
                    features=train_batch['features_pos'].to(self._device)
                )

                batch_score_neg = self._model(
                    input_ids=train_batch['input_ids_neg'].to(self._device),
                    attention_mask=train_batch['attention_mask_neg'].to(self._device),
                    token_type_ids=train_batch['token_type_ids_neg'].to(self._device),
                    features=train_batch['features_neg'].to(self._device)
                )
                ...
                batch_score = self._model(
                    ...
                    attention_mask=train_batch['attention_mask'].to(self._device),
                    token_type_ids=train_batch['token_type_ids'].to(self._device),
                    features=train_batch['features'].to(self._device)
                )
