...
def write_to_file(data: Dict[str, List[str]], save: str):
...
def create_data(data_file: str, context: str, save: str, entity_list):
    ...
    for example in tqdm(utils.aspect_link_examples(data_file), total=total):
        if context == 'para':
            entities = utils.get_entity_ids_only(example.context.paragraph.entities)
        else:
            entities = utils.get_entity_ids_only(example.context.sentence.entities)

        entities.extend(get_all_aspect_entities(example.candidate_aspects))
        entities = set(entities).intersection(set(entity_list))
        result[example.id] = list(entities)


    print('Writing to file...')
    write_to_file(result, save)
    print('[Done].')
